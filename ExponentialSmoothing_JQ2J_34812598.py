...
# Define file path
file_path = "JQ2Jdata_34812598.xlsx"
...
plt.plot(data['Date'], data['JQ2J'], marker='o', color='green', linestyle='-', linewidth=2, markersize=3)
plt.title('Turnover of Manufacturing and Business sector')
plt.xlabel('Year')
plt.ylabel('Turnover')
...
plt.subplot(4, 1, 4)
plt.plot(data.index, data['JQ2J'], label='Original', linestyle=':', color='orange')
plt.legend()
...
# Plot scatter plot
plt.figure(figsize=(10, 6))
plt.scatter(data['Year'], data['JQ2J'], color='blue', alpha=0.5)
plt.title('Scatter Plot of Turnover Over Time')
plt.xlabel('Year')
plt.ylabel('Turnover')
plt.grid(True)
plt.tight_layout()
...
# Plot ACF
plt.figure(figsize=(10, 6))
plot_acf(data['JQ2J'], lags=50, ax=plt.gca())
...
fit1 = ExponentialSmoothing(data['JQ2J'], seasonal_periods=12,seasonal='add').fit()

# Fit Holt-Winters model with multiplicative seasonality
fit2 = ExponentialSmoothing(data['JQ2J'], seasonal_periods=12,seasonal='mul').fit()
...
model = ExponentialSmoothing(data['JQ2J'], seasonal='add', seasonal_periods=12)
fit_model = model.fit()
